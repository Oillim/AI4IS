...
def train_client(server_ip, client_index, attack=None, reg_method=None):
    ...
    if attack == 'data':
        (x_train, y_train) = fe.poison_dataset(x_train, y_train, 1, poison_ratio=0.5)
    ...
    print(f"Validation accuracy: {np.max(history.history['val_accuracy']):.4f}. Validation loss: {np.mean(history.history['val_loss']):.4f}")
    plt.figure()
